...
**Current state:** chain live across machines, 831 ae-node tests green (+ 97 app
...
- ~~**[HIGH / determinism] Supportive and ambient payouts at commit are computed from node-local tag tables**~~ **FIXED (audit #16).** Products, spaces, and supportive/ambient tag submissions are now one chain-ordered lane: `tagging/tagging-operation.ts` (schema v20, `blocks.tagging_operations` + `pending_tagging_operations`). A signed op is verified + queued + gossiped (`new_tagging_op`) and applied deterministically at commit — before `applyChainDayCycle`, so a submit in the same block as the day boundary is finalized this cycle. All four route POSTs (`/tags/products|spaces|supportive|ambient`) return `{status:'pending'}`; the row appears once the block commits. Deterministic ids replace `uuid()` (signature-derived, one per tag row via index), block timestamp replaces `Date.now()`, re-submission stays DELETE-active-then-INSERT (last-writer-wins by block order), and finalize gained `ORDER BY` for iteration determinism. **The load-bearing subtlety:** tagging ops carry free text (names) and arrays, so the canonical signed bytes are a JSON positional array, NOT the pipe-join vouch/miner use — a `|` in a product name would otherwise break signer/verifier agreement. ae-app's four signers build byte-identical strings, pinned by `ae-app/src/lib/crypto.test.ts`. Missing refs (product/space/manufacturer/entity) are rejected at validate, tolerated (whole-op skip) at apply. Proven by `tests/tagging-operation-determinism.test.ts` (12 cases incl. the pipe/quotes guarantee and two-node convergence) and the 3-validator LAN test (product + supportive tag converge across nodes; state roots identical). The tag VALUE math (`finalizeSupportiveTags`/`finalizeAmbientTags`) was already deterministic and needed no change. An adversarial multi-agent review of the first cut caught three consensus bugs, now fixed and regression-tested: (1) `applyTaggingOperation` delegated to `submit*`/`registerSpace` which THROW on cap/minute/collectionRate violations — on the commit path with no pre-commit applicability filter, a validly-signed invalid op would fail-stop every node and brick the chain; apply now mirrors the tolerate-at-apply discipline of the other lanes (a pure `intrinsicSkip`, never throws). (2) The ops-hash sorts but apply used payload order, so a Byzantine proposer could ship two orderings that hash identically and fork followers; apply now uses `orderTaggingOperationsForApply` (canonical order matching the hash). (3) Apply skipped a register on a missing manufacturer/entity ACCOUNT, but account existence is not chain-pure (accounts gossip ahead of registration), so that skip forked nodes; the ref is now stored verbatim and finalization decides payout. Known remaining (shared with the vouch/miner/panel lanes, non-forking): a superseded submit op re-proposed after its replacement can resurrect stale rows — a uniform "committed-op replay" guard across all lanes is the follow-up.
