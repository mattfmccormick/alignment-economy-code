...
- **White-paper alignment audit (`docs/FOUNDATION_BUILD_PLAN.md` Group W, July 5):** a claim-by-claim pass over the July white paper. Almost every mechanic matches to the exact constant. Five divergences logged: **W1** duplicate-account verdict (earliest survives + 2× overlap penalty) is missing — every guilty verdict is treated identically (building now); **W2** the wallet doesn't gross up the percentHuman spend discount (protocol is correct, UI shorts the recipient); **W3** validators are stake-weighted in code but the WP says accuracy-weighted (needs a decision); **W4** follower catch-up sync has no retry; **W5** honesty pass — the ZK nullifier is a plain file hash, the enrollment fee isn't in code, smart-contract Supportive collection is a placeholder, and the 11-miner ramp-up review is unbuilt.
