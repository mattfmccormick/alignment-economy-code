...
### Auth-audit findings still open (not blocking 2-person test, but real)

While auditing for similar gaps, found these unauthenticated write endpoints. None are currently exploited because the network has 2-3 trusted participants, but they all need fixing before public beta. Same pattern as the vouch + tag fixes (add `authMiddleware(db)`, read accountId from `req.accountId`, 403 on mismatch).
...
- **`POST /tags/products`** + **`POST /tags/spaces`** — anyone can register products/spaces in someone else's name. Low impact (just author attribution).
