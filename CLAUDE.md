...
- [x] **Onboarding tuned for non-technical users.** ~~Today's flow assumes you know what a recovery phrase is. Add education, not just a 12-word screen.~~ Two new educational gates in `ae-app/src/pages/Onboarding.tsx`. (1) "New here? What is this?" link on the welcome screen opens a `what-is-ae` screen with 5 plain-language cards (everyone gets points daily, most expire daily, earned points last forever, real humans only, no bank runs this) — the 60-second AE primer for someone who has never used a wallet. (2) New `learn-recovery` step inserted between account creation and the 12-word reveal. Four cards walking through: it's the only key to your account, no one can reset it, write it on paper (no screenshots / no email / no notes app), you'll need it on a new device. The "show me the words" CTA continues to `show-key` only after the user has acknowledged the warnings. Browser-verified: welcome → "What is this?" (5 cards) → back → Create Account → Pick a Network → Solo → "Before you see your phrase" (4 cards) → "show me the words" → "Your Recovery Phrase" (12 words rendered, Account ID shown).
...
- [x] **TypeScript SDK** ~~+ dev portal. So third parties can integrate. Vegas Guy plan Phases 9.5, 9.6.~~ SDK v0.1 done; dev portal is a follow-up (the README + inline doc comments cover the SDK surface for now). New `sdk/` package at the repo root publishes as `@alignmenteconomy/sdk`. Includes `AlignmentEconomyClient` (typed wrappers around `/api/v1/health`, `/accounts`, `/transactions`, `/transactions/:id`, `/network/status`, `/network/blocks`, `/founder/generate-genesis`) + `signTransaction` helper + crypto re-exports (`generateKeyPair`, `deriveAccountId`, `signPayload`, `verifyPayload`, `newMnemonic`, `mnemonicToKeypair`). Bigints round-trip as base-10 strings. Errors throw `SDKError` with `.code` + `.httpStatus`. 8/8 smoke tests pass against a real spawned ae-node (boot, health, account create/get, network status, mnemonic round-trip, sign+verify, founder genesis ceremony, getTransaction NOT_FOUND). Court / verification / mining / vouching / tag endpoints land in v0.2.
