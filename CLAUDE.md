...
- **WebSocket subscribe has no authentication.** Backend `websocket.ts` defines a signed `subscribe` handshake, but verify the wallet/miner clients actually call it (audit suggests `wsClient` may open the WS without auth). Anyone who knows an accountId could otherwise read its balance updates, court filings, and verification events.
