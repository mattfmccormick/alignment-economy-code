...
- [x] **TypeScript SDK** ~~+ dev portal. So third parties can integrate. Vegas Guy plan Phases 9.5, 9.6.~~ SDK v0.2 done; dev portal is a follow-up (the README + inline doc comments cover the SDK surface for now). New `sdk/` package at the repo root publishes as `@alignmenteconomy/sdk`. v0.1 covered `AlignmentEconomyClient` (typed wrappers around `/api/v1/health`, `/accounts`, `/transactions`, `/transactions/:id`, `/network/status`, `/network/blocks`, `/founder/generate-genesis`) + `signTransaction` helper + crypto re-exports (`generateKeyPair`, `deriveAccountId`, `signPayload`, `verifyPayload`, `newMnemonic`, `mnemonicToKeypair`). v0.2 adds the read-only court / miner / tag surface a third-party explorer or analytics consumer would want without needing to sign anything: `getCases`, `getCase(id)`, `getJuryDuty(accountId)`, `getMyCases(accountId)`, `getMinerStatus(accountId)`, `getVouches(accountId)`, `getProducts`, `getSpaces`, `getCurrentDay`. New types exported alongside (`CourtCase`, `CaseType`, `CaseLevel`, `CaseStatus`, `Verdict`, `MinerStatus`, `Vouch`, `VouchesForAccount`, `Product`, `Space`). Bigints round-trip as base-10 strings. Errors throw `SDKError` with `.code` + `.httpStatus`. 12/12 smoke tests pass against a real spawned ae-node (the original 8 + 4 new v0.2 reads). Write-side helpers for court argument submission, vouching, and jury voting land in v0.3 once the signing payload formats are documented.
