...
- [ ] **Wire `main.cjs` to honor network choice.** Today's `ae-app/electron/main.cjs` and `ae-miner/electron/main.cjs` always boot ae-node in single-validator authority mode regardless of the user's network-mode choice. Tasks 2-4 save the chosen mode + genesis + keystore in localStorage, but main.cjs ignores that and spawns ae-node with hardcoded defaults. This task: read the wallet's localStorage on Electron boot (or have the renderer signal main via IPC), write the genesis spec + keystore to userData, then spawn ae-node with `AE_GENESIS_CONFIG_PATH` + `AE_NODE_KEY_PATH` + `AE_BFT_LOCAL_ACCOUNT_ID` + `AE_CONSENSUS_MODE=bft` set. Until this lands, "Start new" and "Join existing" save the right intent but the bundled node runs in solo mode under the hood, so no real peering happens.
