...
- **`POST /miners/vouches` has no auth middleware.** Surfaced by the SDK v0.3 work — the `voucherId` in the request body is taken at face value, with no signature check linking it to the calling client. Anyone can vouch on someone else's behalf. The vouch still costs the body-supplied account real earned balance, so it's not free, but it lets any third party drain a victim's balance into vouches they didn't authorize. Add `authMiddleware(db)` + verify `req.accountId === voucherId` on the route. Small surgical fix.
