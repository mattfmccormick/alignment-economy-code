...
- **`POST /miners/register`** — anyone can claim someone else's account as a miner. Probably benign (the victim just gets registered as a verifier whether they wanted to or not), but weird and noisy.
- **`POST /miners/evidence`** — anyone can submit verification evidence for any account. Lets a third party fabricate fake evidence in someone else's name; tier-2 reviewers see fake submissions attributed to the victim.
- **`POST /miners/vouch-requests`** + **`PUT /miners/vouch-requests/:id`** — anyone can create or accept vouch requests on behalf of others. Combined with the now-fixed `/miners/vouches` gap this was much worse; auth on `/miners/vouches` blocks the actual stake, but the request-spam vector remains.
