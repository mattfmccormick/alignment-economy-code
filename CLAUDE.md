...
Real protocol features the whitepaper requires. Milestone 1 ships without them; they're additive. All six are now done — Phase 67 in-person co-sign, Phase 69 inheritance + dead-man-switch, Phase 70 smart contract DSL, the block explorer at `explorer/`, the TypeScript SDK at `sdk/`, and Phase 68 protocol treasury.
...
### Auth-audit findings (3 closed, 3 remaining low-impact)

While auditing for similar gaps to the original `/miners/vouches` finding, found these unauthenticated write endpoints. The high and medium-impact ones are now closed. The remaining three are low-impact (no economic effect today) but should still be tightened before public beta — same pattern (add `authMiddleware(db)`, read accountId from `req.accountId`, 403 on mismatch).

**Closed:**

- ~~**`POST /miners/register`** — anyone can claim someone else's account as a miner.~~ Fixed alongside the rest of the miner write surface. authMiddleware now enforces that the registrant signs the request; back-compat shim accepts a top-level body accountId but rejects with 403 ACCOUNT_MISMATCH on disagreement.
- ~~**`POST /miners/evidence`** — anyone can submit verification evidence for any account.~~ Fixed. Same authMiddleware + ACCOUNT_MISMATCH shape. Wallet's `Verify.tsx` now signs `{evidenceTypeId, evidenceHash}` with the owner's private key before posting.
- ~~**`POST /miners/vouch-requests`** + **`PUT /miners/vouch-requests/:id`** — anyone can create or accept vouch requests on behalf of others.~~ Fixed. POST is auth'd: the requestor (signed account) becomes fromId, body fromId is back-compat (403 FROM_MISMATCH on disagreement). PUT is auth'd AND ownership-checked: only the request's recipient (`toId`) can respond, otherwise 403 NOT_REQUEST_RECIPIENT. Added `findVouchRequestById` to `IVerificationStore` + `SqliteVerificationStore` to support the recipient check. Wallet (`Verify.tsx`) and miner (`Vouch.tsx`, `Login.tsx`) call sites all now sign the appropriate payload.

**Still open (low impact — won't block 2-person testing):**

- **`POST /tags/products`** + **`POST /tags/spaces`** — anyone can register products/spaces in someone else's name. Low impact (just author attribution; no economic redirect).
- **`POST /recurring/`** + **`PUT /recurring/:id`** + **`DELETE /recurring/:id`** — vestigial. The schema has the table but no executor wires the day cycle to fire recurring rows, so these are write-only with no economic effect today. Will become a real theft vector the moment an executor lands. Add auth before that wiring.
- **`POST /contacts/`** + 3 contact-list write methods — manage another account's contact list. Low impact (UI annoyance, no economic effect).
