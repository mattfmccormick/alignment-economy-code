...
- **Installer publisher metadata + code signing.** Today the Windows installer triggers a SmartScreen "Unknown publisher" warning. Two parts:
  - **(Free, ~5 min, do this anytime):** Set `author` in package.json + `companyName` in the build block on both apps to "The Alignment Economy Foundation". Adds proper Publisher / Company / Copyright fields to the .exe metadata so Windows Properties → Details and Add/Remove Programs show the right name. SmartScreen still warns because there's no signature, but the warning dialog reads "Publisher: The Alignment Economy Foundation" instead of "Unknown publisher" once the user clicks "More info."
