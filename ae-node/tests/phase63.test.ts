...
describe('Phase 63: per-block fee distribution (WP economics)', () => {
  it('distributes 20/80 between Tier 1 and Tier 2 (WP v2)', () => {
    const db = freshDb();
  ...
  it('Tier 2 split: 60% lottery winner, 40% baseline', () => {
    ...
    // No tier 1 miners, all 100 goes to tier 2.
    // 60% lottery (60), 40% baseline (40) split across 3 miners.
    const dist = distributeFeesPublicLottery(db, 1, 'fixed-block-hash', totalFees);
    assert.ok(dist);
    assert.equal(dist!.tier1Pool, 0n);
    assert.equal(dist!.tier2Pool, pts(100));
    ...
    // Integer division remainder (1 base unit) goes to the first miner in
    // the array, which may or may not be the lottery winner.
    const winnerId = dist!.lotteryWinnerId!;
    const baselinePerMiner = pts(40) / 3n;
    const winningPayout = baselinePerMiner + pts(60);
    for (const m of [t2a, t2b, t2c]) {
      const acct = getAccount(db, m.accountId)!;
      const base = pts(1000) + (m.minerId === winnerId ? winningPayout : baselinePerMiner);
      const diff = acct.earnedBalance - base;
      assert.ok(diff >= 0n && diff <= 1n, `miner ${m.minerId} payout off by ${diff}`);
    }
    db.close();
  });
